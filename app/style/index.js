  ...
  return createMap({
    ...
    .style('landcover')
      .rule({ filter: types(['forest', 'wood']) })
        .borderedPolygonSymbolizer('hsl(110, 45%, 75%)')
      .rule({ filter: "[type] = 'farmland'" })
        .borderedPolygonSymbolizer('hsl(60, 70%, 95%)')
      .rule({ filter: types(['meadow', 'grassland', 'park', 'cemetery']) })
        .borderedPolygonSymbolizer('hsl(100, 85%, 85%)')
      .rule({ filter: "[type] = 'heath'" })
        .borderedPolygonSymbolizer('hsl(80, 70%, 80%)')
    ...
    .style('feature_point_names', { filterMode: 'first' })
      ...
      .rule({ filter: types(['cave_entrance', 'viewpoint', 'mine', 'adit', 'mineshaft']), minZoom: 14 })
    ...
    .style('building_names', { filterMode: 'first' })
      .rule({ minZoom: 14, filter: types(['church', 'chapel', 'catedhral', 'temple', 'basilica']) })
        .textSymbolizer({ ...fontDflt, placement: 'interior', dy: -10 }, '[name]')
      .rule({ minZoom: 15 })
