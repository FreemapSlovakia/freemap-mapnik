const builder = require('xmlbuilder');

function cc2snake(obj) {
  const res = {};
  Object.keys(obj).forEach((key) => {
    res[key.replace(/[A-Z]/, (x) => '-' + x.toLowerCase())] = obj[key];
  });
  return res;
}

const zoomDenoms = [
  250000000000,
  500000000,
  200000000,
  100000000,
  50000000,
  25000000,
  12500000,
  6500000,
  3000000,
  1500000,
  750000,
  400000,
  200000,
  100000,
  50000,
  25000,
  12500,
  5000,
  2500,
];

function createMap(atts, { dbParams } = {}) {
  const mapEle = builder.begin().ele('Map', cc2snake(atts));

  if (dbParams) {
    const dsEle = mapEle.ele('Datasource', { name: 'db' });
    Object.keys(dbParams).forEach((name) => {
      dsEle.ele('Parameter', { name }, dbParams[name]);
    });
  }

  const map = {
    addStyle(name, atts = {}) {
      const styleEle = mapEle.ele('Style', { name, ...cc2snake(atts) });
      const style = {
        doInStyle(cb) {
          cb(style);
          return style;
        },
        addRule({ filter, minZoom, maxZoom } = {}) {
          const ruleEle = styleEle.ele('Rule');
          if (filter) {
            ruleEle.ele('Filter', {}, filter);
          }
          if (typeof minZoom === 'number') {
            ruleEle.ele('MinScaleDenominator', {}, zoomDenoms[minZoom]);
          }
          if (typeof maxZoom === 'number') {
            ruleEle.ele('MaxScaleDenominator', {}, zoomDenoms[maxZoom]);
          }
          const ascendents = { style, map };
          const rule = {
            addFilter(filter) {
              const filterEle = ruleEle.ele('Filter', {}, filter);
              return { filterEle, ...rule, ...ascendents };
            },
            addPolygonSymbolizer(atts = {}) {
              const polygonSymbolizerEle = ruleEle.ele('PolygonSymbolizer', cc2snake(atts));
              return { polygonSymbolizerEle, ...rule, ...ascendents };
            },
            addLineSymbolizer(atts = {}) {
              const lineSymbolizerEle = ruleEle.ele('LineSymbolizer', cc2snake(atts));
              return { lineSymbolizerEle, ...rule, ...ascendents };
            },
            addMarkersSymbolizer(atts = {}) {
              const markersSymbolizerEle = ruleEle.ele('MarkersSymbolizer', cc2snake(atts));
              return { markersSymbolizerEle, ...rule, ...ascendents };
            },
            addTextSymbolizer(atts = {}, text) {
              const textSymbolizerEle = ruleEle.ele('TextSymbolizer', cc2snake(atts), text);
              return { textSymbolizerEle, ...rule, ...ascendents };
            },
            addRasterSymbolizer(atts = {}) {
              const rasterSymbolizerEle = ruleEle.ele('RasterSymbolizer', cc2snake(atts));
              return { rasterSymbolizerEle, ...rule, ...ascendents };
            },

            addBorderedPolygonSymbolizer(color) {
              const polygonSymbolizerEle = ruleEle.ele('PolygonSymbolizer', cc2snake({ fill: color }));
              const lineSymbolizerEle = ruleEle.ele('LineSymbolizer', cc2snake({ stroke: color, strokeWidth: 1 }));
              return { polygonSymbolizerEle, lineSymbolizerEle, ...rule, ...ascendents };
            },
            ruleEle,
            ...style,
          };
          return rule;
        },
        styleEle,
        ...map,
      };
      return style;
    },
    addSqlLayer(layerName, styleName, sql) {
      const layerEle = mapEle.ele('Layer', { name: layerName });
      layerEle.ele('StyleName', {}, styleName);
      layerEle.ele('Datasource', { base: 'db' })
        .ele('Parameter', { name: 'table' }, `(${sql}) as foo`);
      return this;
    },
    addLayer(layerName, styleName, dsParams) {
      const layerEle = mapEle.ele('Layer', { name: layerName });
      layerEle.ele('StyleName', {}, styleName);
      const dsEle = layerEle.ele('Datasource')
      Object.keys(dsParams).forEach((name) => {
        dsEle.ele('Parameter', { name }, dsParams[name]);
      });
      return this;
    },
    mapEle,
    stringify(formattingOptions = { pretty: true }) {
      return mapEle.end(formattingOptions)
    },
  };

  return map;
}

module.exports = {
  cc2snake,
  createMap,
};
