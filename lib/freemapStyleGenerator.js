  ...
  return createMap({
    ...
    .addStyle('Water-line')
      .addRule({ filter: "[type] = 'river'" })
        .addLineSymbolizer({ stroke: colors.water, strokeWidth: 1, smooth: 1 })
      .addRule({ filter: "[type] <> 'river'" })
        .addLineSymbolizer({ stroke: colors.water, strokeWidth: 0.5, smooth: 1 })
    .addStyle('tracks')
      .addRule({ filter: "[type] = 'residential' or [type] = 'service' or [type] = 'unclassified' or [type] = 'road' or [type] = 'primary' or [type] = 'secondary' or [type] = 'tertiary' or [type] = 'motorway' or [type] = 'trunk'" })
        .addLineSymbolizer({ stroke: '#ffffff', strokeWidth: 3, strokeOpacity: 0.5, smooth: 1 })
        .addLineSymbolizer({ stroke: '#804040', strokeWidth: 1.2, smooth: 1 })
      .addRule({ filter: "[type] = 'path'" })
        .addLineSymbolizer({ stroke: '#ffffff', strokeWidth: 3, strokeOpacity: 0.5, smooth: 1 })
        .addLineSymbolizer({ stroke: '#804040', strokeWidth: 1.2, strokeDasharray: '2,2', smooth: 1 })
      .addRule({ filter: "[type] = 'footway'" })
        .addLineSymbolizer({ stroke: '#ffffff', strokeWidth: 3, strokeOpacity: 0.5, smooth: 1 })
        .addLineSymbolizer({ stroke: '#804040', strokeWidth: 1.2, strokeDasharray: '3,1', smooth: 1 })
        ...
        [undefined, '8,2', '6,4', '4,6', '2,8', '3,7,7,3'].forEach((strokeDasharray, i) => {
          style
            .addRule({ filter: `[type] = 'track' and [tracktype] = ${i === 5 ? `''` : `'grade${i + 1}'`}` })
              .addLineSymbolizer({ stroke: colors.track, strokeWidth: 1.2, strokeDasharray, smooth: 1 })
        });
      })
    .addStyle('buildings')
      .addRule()
    ...
    .addStyle('contours', { opacity: 0.33 })
      .addRule({ maxZoom: 13, filter: "([height] % 100 = 0) and ([height] != 0)" })
        .addLineSymbolizer({ stroke: colors.contour, strokeWidth: 0.3, smooth: 1 })
      .addRule({ maxZoom: 14, filter: "([height] % 10 = 0) and ([height] != 0)" })
        .addLineSymbolizer({ stroke: colors.contour, strokeWidth: 0.2, smooth: 1 })
