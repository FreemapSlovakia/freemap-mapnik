  ...
  return createMap({
    ...
    .addStyle('naturalpoints')
      ...
      .addRule({ filter: "[type] = 'spring'", minZoom: 12 })
        ...
        .addTextSymbolizer({ size: 10, faceName: 'DejaVu Sans Book', fill: 'black', haloFill: 'white', haloRadius: 1, dy: -8 }, '[name]')
    .addStyle('placenames', { opacity: 0.66 })
      .addRule({ filter: "[type] = 'city' or [type] = 'town'" })
        .addTextSymbolizer({ size: 30, faceName: 'DejaVu Sans Book', fill: 'black', haloFill: 'white', haloRadius: 1 }, '[name]')
      .addRule({ filter: "[type] = 'village' or [type] = 'suburb'", minZoom: 10 })
        .addTextSymbolizer({ size: 20, faceName: 'DejaVu Sans Book', fill: 'black', haloFill: 'white', haloRadius: 1 }, '[name]')
      .addRule({ filter: "[type] = 'hamlet'", minZoom: 11 })
        .addTextSymbolizer({ size: 15, faceName: 'DejaVu Sans Book', fill: 'black', haloFill: 'white', haloRadius: 1 }, '[name]')
    .addStyle('routes')
    ...
    .addSqlLayer('placenames',
      `select name, type, geometry from import.osm_places order by z_order`,
      { clearLabelCache: 'on' })
    .stringify();
}
  ...
  return (style) => {
    ...
    for (let colorIdx = 0; colorIdx < colors.length; colorIdx++) {
      ...
      m.forEach((combs, ones) => {
        ...
        for (let a = 0; a < 2; a++) {
          style
            .addRule({ filter, minZoom: a === 0 ? 11 : 9, maxZoom: a === 1 ? undefined : 10 })
              .addLineSymbolizer({
                stroke: colors[colorIdx],
                strokeWidth: 2,
                strokeLinejoin: 'round',
                offset: ((a == 0 ? 4 : 1) + ones * 2) * (type === 'hiking' ? 1 : -1),
