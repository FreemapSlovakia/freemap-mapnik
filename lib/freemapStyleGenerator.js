...
module.exports = function generateFreemapStyle() {
  return createMap({
    ...
    .addStyle('higwayGlows')
      .addRule({ filter: "[type] = 'path' or [type] = 'footway'", minZoom: 12 })
    ...
    .addSqlLayer('Water-area',
      `select geometry, name, type from osm_waterareas`)
    ...
    .addSqlLayer('routes',
      `select geometry, concat('/', string_agg("osmc:symbol", '/')) as osmc_symbol, concat('/', string_agg(colour, '/'), '/') as colour, osm_routes.type from osm_route_members join osm_routes using(osm_id) group by member, geometry, osm_routes.type`)
