...
function generateFreemapStyle({
  ...
  return createMap({
    ...
    .style('water_line')
      ...
      .typesRule(10, 11, 'river')
        .lineSymbolizer({ stroke: colors.water, strokeWidth: 2.2, strokeOpacity: '1 - [tunnel] / 0.6', strokeDasharray: '[dasharray]' })
      .typesRule(12, 'river')
        .lineSymbolizer({ stroke: colors.water, strokeWidth: 2.2, strokeOpacity: '1 - [tunnel] / 0.6', strokeDasharray: '[dasharray]', smooth: 0.5 })
      .rule({ filter: "[type] <> 'river'", minZoom: 12 })
        .lineSymbolizer({ stroke: colors.water, strokeWidth: 1.2, strokeOpacity: '1 - [tunnel] / 0.6', strokeDasharray: '[dasharray]', smooth: 0.5 })
      .rule({ minZoom: 14 })
    ...
    .style('water_line_names')
      .typesRule(12, 'river')
        .textSymbolizer(font().water().line(400).end({ characterSpacing: 2, simplifyAlgorithm: 'douglas-peucker', simplify: 10, maxCharAngleDelta: 30 }), '[name]')
      .rule({ minZoom: 14, filter: "[type] <> 'river'" })
        .textSymbolizer(font().water().line(300).end({ characterSpacing: 2, simplifyAlgorithm: 'douglas-peucker', simplify: 10, maxCharAngleDelta: 30 }), '[name]')
    .style('fixmes')
      .rule()
